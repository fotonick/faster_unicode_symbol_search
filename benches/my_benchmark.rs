...
use faster_unicode_symbol_search::symbols::{from_string, Symbols};
...
pub fn bench_symbols(c: &mut Criterion) {
    ...
    group.bench_with_input(
        BenchmarkId::new("Symbols::from_string", "src/symbols.txt"),
        &buffer,
        |b, s| b.iter(|| from_string(black_box(s))),
    );
...
pub fn bench_search(c: &mut Criterion) {
    ...
    for input in ["rarr", "µ", "capital delta"].iter() {
        group.bench_with_input(
            BenchmarkId::new("Symbols::search_symbols", input),
            input,
            |b, s| b.iter(|| symbols.search_symbols(black_box(s))),
        );
    }
...
pub fn bench_program(c: &mut Criterion) {
    ...
    for input in ["rarr", "µ", "capital delta"].iter() {
        group.bench_with_input(
            BenchmarkId::new("target/release/fuss", input),
            input,
            |b, input| b.iter(|| command.arg(black_box(input)).output().unwrap()),
        );
    }
